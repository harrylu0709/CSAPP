 ...
 * https://github.com/ryanfarr01/Malloc-Lab/blob/master/mm.c#L108
 */
#include <stdio.h>
#include <stdlib.h>
#include <assert.h>
...
static void place(void *ptr, size_t asize)
{
    ...
    if((csize - asize) >= BLOCK) 
    {
        ...
        PUT_PTR(nextB, NULL); //next
        PUT_PTR(nextB + WSIZE, NULL); //prev
...
static void *coalesce(void *ptr)
{
    ...
    PUT_PTR(GET_NEXTP(ptr), NULL); //place null for next
    PUT_PTR(GET_PREVP(ptr), NULL); //place null for prev
...
void find_and_place(void * ptr)
{
    ...
    {
        PUT_PTR(GET_PREVP(current), ptr); //h.prev = n
        PUT_PTR(GET_NEXTP(ptr), current); //n.next = h;
        free_list[listIndex] = ptr;
...
static void remove_node_references(void *ptr)
{
    ...
    {
        PUT_PTR(GET_NEXTP(GET_PREV(ptr)), GET_NEXT(ptr)); //n.prev.next = n.next;
    }
    else
    {
        void** h = is_head(ptr);
        PUT_PTR(h,GET_NEXT(ptr)); //no previous implies it was a head
    }
    if(GET_NEXT(ptr) != NULL)
    {
        PUT_PTR(GET_PREVP(GET_NEXT(ptr)), GET_PREV(ptr)); //n.next.prev = n.prev;
    }

    PUT_PTR(GET_NEXTP(ptr), NULL); //replace next
    PUT_PTR(GET_PREVP(ptr), NULL); //replace prev
}
...
static void *extend_heap(size_t words)
{
    ...
    PUT_PTR(bp, NULL); //next
    PUT_PTR(bp + WSIZE, NULL); //prev
